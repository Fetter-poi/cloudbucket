...
router.route('/info')
  .get(function (req, res, next) {
    Q.all([
      fileService.totalSize(),
      fileService.countFiles()
    ]).then(function (results) {
      var totalSize = results[0];
      var count = results[1];
      res.send({
        totalSize: totalSize,
        count: count
...
router.route('/upload')
  .post(function (req, res, next) {
    ...
    form.parse(req, function (err, fields, data) {
      ...
      if (fields.lastModified && fields.lastModified.length) {
        createdAt = Number(fields.lastModified[0]);
      }
      Q.all(data.file.map(function (file) {
        return fileService.upload(file, createdAt);
      })).then(function (results) {
        if (results.length) {
          var doc = results[0];
          if (doc) {
